...
void Controller::run()
{
    ...
    command_fcns.insert(make_pair("default",bind(&View::set_defaults, view)));
    command_fcns.insert(make_pair("show", bind(&View::draw, view)));
    command_fcns.insert(make_pair("status",
                                   bind(&Model::describe, g_Model_ptr)));
    command_fcns.insert(make_pair("go", bind(&Model::update, g_Model_ptr)));
    ...
    while(true) {//run until "quit" has been read
        try {
            ...
            else if(g_Model_ptr->is_agent_present(cmd)) {
                //in other words, if the input is an agent
                Agent* agent = g_Model_ptr->get_agent_ptr(cmd);
                if(!agent->is_alive()) {
...
void Controller::build()
{
    ...
    Structure* new_struct = create_structure(new_obj.name, new_obj.type,
                                             get_Point());
    g_Model_ptr->add_structure(new_struct);
}
...
void Controller::work(Agent* agent)
{
    string source, dest;
    cin >> source >> dest;
    agent->start_working(g_Model_ptr->get_structure_ptr(source),
                         g_Model_ptr->get_structure_ptr(dest));
}
...
void Controller::attack(Agent* agent)
{
    string target;
    cin >> target;
    agent->start_attacking(g_Model_ptr->get_agent_ptr(target));
}
